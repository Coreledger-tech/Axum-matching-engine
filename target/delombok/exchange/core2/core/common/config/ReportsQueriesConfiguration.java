...
public final class ReportsQueriesConfiguration {
    ...
    public static ReportsQueriesConfiguration createStandardConfig(final Map<Integer, Class<? extends ReportQuery<?>>> customReports) {
        ...
        return new ReportsQueriesConfiguration(Collections.unmodifiableMap(reportConstructors), Collections.unmodifiableMap(binaryCommandConstructors));
    }

    private static void addQueryClass(final Map<Integer, Constructor<? extends ReportQuery<?>>> reportConstructors, final int reportTypeCode, final Class<? extends ReportQuery<?>> reportQueryClass) {
    ...
    private static void addBinaryCommandClass(Map<Integer, Constructor<? extends BinaryDataCommand>> binaryCommandConstructors, BinaryCommandType type, Class<? extends BinaryDataCommand> binaryCommandClass) {
        try {
    ...
    private ReportsQueriesConfiguration(final Map<Integer, Constructor<? extends ReportQuery<?>>> reportConstructors, final Map<Integer, Constructor<? extends BinaryDataCommand>> binaryCommandConstructors) {
    ...
    public String toString() {
        return "ReportsQueriesConfiguration{reportConstructors=[" + reportToString(reportConstructors) + "], binaryCommandConstructors=[" + reportToString(binaryCommandConstructors) + "]}";
    }

    private static String reportToString(final Map<Integer, ? extends Constructor<?>> mapping) {
        return mapping.entrySet().stream().map(entry -> String.format("%d:%s", entry.getKey(), entry.getValue().getDeclaringClass().getSimpleName())).collect(Collectors.joining(", "));
    }
