...
public final class ExchangeApi {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ExchangeApi.class);
...
//                || cmd.command == OrderCommandType.BINARY_DATA_QUERY) {
    ...
    private <T extends ApiCommand, R> CompletableFuture<R> submitCommandAsync(final EventTranslatorOneArg<OrderCommand, T> translator, final T apiCommand, final Function<OrderCommand, R> responseTranslator) {
    ...
    public CompletableFuture<CommandResultCode> submitBinaryDataAsync(final BinaryDataCommand data) {
        ...
        publishBinaryData(OrderCommandType.BINARY_DATA_COMMAND, data, data.getBinaryCommandTypeCode(), (int) System.nanoTime(),  // can be any value because sequence is used for result identification, not transferId
    ...
    public <R> CompletableFuture<R> submitBinaryCommandAsync(final BinaryDataCommand data, final int transferId, final Function<OrderCommand, R> translator) {
        final CompletableFuture<R> future = new CompletableFuture<>();
        publishBinaryData(ApiBinaryDataCommand.builder().data(data).transferId(transferId).build(), seq -> promises.put(seq, orderCommand -> future.complete(translator.apply(orderCommand))));
        return future;
    }
    ...
    public <R> CompletableFuture<R> submitQueryAsync(final ReportQuery<?> data, final int transferId, final Function<OrderCommand, R> translator) {
        final CompletableFuture<R> future = new CompletableFuture<>();
        publishQuery(ApiReportQuery.builder().query(data).transferId(transferId).build(), seq -> promises.put(seq, orderCommand -> future.complete(translator.apply(orderCommand))));
        return future;
    }
    ...
    public <Q extends ReportQuery<R>, R extends ReportResult> CompletableFuture<R> processReport(final Q query, final int transferId) {
        return submitQueryAsync(query, transferId, cmd -> query.createResult(OrderBookEventsHelper.deserializeEvents(cmd).values().parallelStream().map(Wire::bytes)));
    }

    public void publishBinaryData(final ApiBinaryDataCommand apiCmd, final LongConsumer endSeqConsumer) {
        publishBinaryData(OrderCommandType.BINARY_DATA_COMMAND, apiCmd.data, apiCmd.data.getBinaryCommandTypeCode(), apiCmd.transferId, apiCmd.timestamp, endSeqConsumer);
    }

    public void publishQuery(final ApiReportQuery apiCmd, final LongConsumer endSeqConsumer) {
        publishBinaryData(OrderCommandType.BINARY_DATA_QUERY, apiCmd.query, apiCmd.query.getReportTypeCode(), apiCmd.transferId, apiCmd.timestamp, endSeqConsumer);
    }

    private void publishBinaryData(final OrderCommandType cmdType, final WriteBytesMarshallable data, final int dataTypeCode, final int transferId, final long timestamp, final LongConsumer endSeqConsumer) {
        final long[] longsArrayData = SerializationUtils.bytesToLongArrayLz4(lz4Compressor, BinaryCommandsProcessor.serializeObject(data, dataTypeCode), LONGS_PER_MESSAGE);
...
//        log.debug("longsArrayData[{}] n={}", longsArrayData.length, totalNumMessagesToClaim);
    ...
    private void publishBinaryMessageFragment(OrderCommandType cmdType, int transferId, long timestamp, LongConsumer endSeqConsumer, long[] longsArrayData, int fragmentSize, int offset, boolean isLastFragment) {
...
//            log.debug("REPLAY seq={} cmd={}", seq, cmd);
    ...
    public void balanceAdjustment(long uid, long transactionId, int currency, long longAmount, BalanceAdjustmentType adjustmentType, Consumer<OrderCommand> callback) {
        ringBuffer.publishEvent(((cmd, seq) -> {
            cmd.command = OrderCommandType.BALANCE_ADJUSTMENT;
    ...
    public void balanceAdjustment(int serviceFlags, long eventsGroup, long timestampNs, long uid, long transactionId, int currency, long longAmount, BalanceAdjustmentType adjustmentType) {
        ringBuffer.publishEvent(((cmd, seq) -> {
            cmd.serviceFlags = serviceFlags;
            cmd.eventsGroup = eventsGroup;
            cmd.command = OrderCommandType.BALANCE_ADJUSTMENT;
    ...
    public long placeNewOrder(int userCookie, long price, long reservedBidPrice, long size, OrderAction action, OrderType orderType, int symbol, long uid, Consumer<OrderCommand> callback) {
    ...
    @java.lang.SuppressWarnings("all")
    public ExchangeApi(final RingBuffer<OrderCommand> ringBuffer, final LZ4Compressor lz4Compressor) {
