...
public class SimpleEventsProcessor implements ObjLongConsumer<OrderCommand> {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(SimpleEventsProcessor.class);
    ...
    private void sendTradeEvents(OrderCommand cmd) {
        ...
        if (firstEvent.eventType == MatcherEventType.REDUCE) {
            final IEventsHandler.ReduceEvent evt = new IEventsHandler.ReduceEvent(cmd.symbol, firstEvent.size, firstEvent.activeOrderCompleted, firstEvent.price, cmd.orderId, cmd.uid, cmd.timestamp);
    ...
    private void sendTradeEvent(OrderCommand cmd) {
        ...
        cmd.processMatcherEvents(evt -> {
            if (evt.eventType == MatcherEventType.TRADE) {
                final IEventsHandler.Trade trade = new IEventsHandler.Trade(evt.matchedOrderId, evt.matchedOrderUid, evt.matchedOrderCompleted, evt.price, evt.size);
                trades.add(trade);
        ...
        if (!trades.isEmpty()) {
            final IEventsHandler.TradeEvent evt = new IEventsHandler.TradeEvent(cmd.symbol, mutableLong.value, cmd.orderId, cmd.uid, cmd.action, takerOrderCompleted.value, cmd.timestamp, trades);
    ...
    private void sendCommandResult(OrderCommand cmd, long seq) {
        switch (cmd.command) {
        case PLACE_ORDER: 
            sendApiCommandResult(new ApiPlaceOrder(cmd.price, cmd.size, cmd.orderId, cmd.action, cmd.orderType, cmd.uid, cmd.symbol, cmd.userCookie, cmd.reserveBidPrice), cmd.resultCode, cmd.timestamp, seq);
            break;
        case MOVE_ORDER: 
    ...
    @java.lang.SuppressWarnings("all")
    public SimpleEventsProcessor(final IEventsHandler eventsHandler) {
        this.eventsHandler = eventsHandler;
    }

    @java.lang.SuppressWarnings("all")
    public IEventsHandler getEventsHandler() {
