...
public interface ISerializationProcessor {
     ...
     */
    boolean storeData(long snapshotId, long seq, long timestampNs, SerializedModuleType type, int instanceId, WriteBytesMarshallable obj);
    ...
    enum SerializedModuleType {
        RISK_ENGINE("RE"), MATCHING_ENGINE_ROUTER("ME");
        final String code;

        @java.lang.SuppressWarnings("all")
        private SerializedModuleType(final String code) {
            this.code = code;
        }
    }

    static boolean canLoadFromSnapshot(final ISerializationProcessor serializationProcessor, final InitialStateConfiguration initStateCfg, final int shardId, final ISerializationProcessor.SerializedModuleType module) {
        if (initStateCfg.fromSnapshot()) {
            final boolean snapshotExists = serializationProcessor.checkSnapshotExists(initStateCfg.getSnapshotId(), module, shardId);
            if (snapshotExists) {
