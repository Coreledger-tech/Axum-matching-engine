...
public final class DiskSerializationProcessor implements ISerializationProcessor {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(DiskSerializationProcessor.class);
...
//    final SingleWriterRecorder hdrRecorderLz4 = new SingleWriterRecorder(Integer.MAX_VALUE, 2);
    public DiskSerializationProcessor(ExchangeConfiguration exchangeConfig, DiskSerializationProcessorConfiguration diskConfig) {
    ...
    public boolean storeData(long snapshotId, long seq, long timestampNs, SerializedModuleType type, int instanceId, WriteBytesMarshallable obj) {
        ...
        try (
            ...
            LZ4FrameOutputStream lz4os = new LZ4FrameOutputStream(bos, LZ4FrameOutputStream.BLOCKSIZE.SIZE_4MB, -1, lz4CompressorSnapshot, XXHashFactory.fastestInstance().hash32(), LZ4FrameOutputStream.FLG.Bits.BLOCK_INDEPENDENCE);
            WireToOutputStream2 wireToOutputStream = new WireToOutputStream2(WireType.RAW, lz4os)) {
        ...
        synchronized (this) {
            // TODO improve format
            try (OutputStream os = Files.newOutputStream(resolveMainLogPath(), StandardOpenOption.CREATE, StandardOpenOption.APPEND)) {
...
//        log.debug("Writing {}", cmd);
        ...
        if (debug) log.debug("LOG {} eventsGroup={} serviceFlags={}", String.format("seq=%d t=%d cmd=%X (%s) ", baseSeq + dSeq, cmd.timestamp, cmdType.getCode(), cmdType), cmd.eventsGroup, cmd.serviceFlags);
        if (cmdType == OrderCommandType.MOVE_ORDER) {
            ...
            if (debug) log.debug("place order seq={} t={} orderId={} symbol={} uid={} price={} reserveBidPrice={} size={} userCookie={} {}/{} actionAndType={}", baseSeq + dSeq, cmd.timestamp, cmd.orderId, cmd.symbol, cmd.uid, cmd.price, cmd.reserveBidPrice, cmd.size, cmd.userCookie, cmd.action, cmd.orderType, actionAndType1);
        ...
        } else if (cmdType == OrderCommandType.ADD_USER || cmdType == OrderCommandType.SUSPEND_USER || cmdType == OrderCommandType.RESUME_USER) {
...
//        log.info("Read total: {} bytes ", totalBytesRead);
    ...
    private void readCommands(final DataInputStream jr, final ExchangeApi api, final MutableLong lastSeq, boolean insideCompressedBlock) throws IOException {
...
//                log.debug("command seq={} {}", lastSeq, cmdType);
                ...
                if (cmdType == OrderCommandType.MOVE_ORDER) {
                    ...
                    final OrderAction orderAction = OrderAction.of((byte) (actionAndType & 1));
                    final OrderType orderType = OrderType.of((byte) ((actionAndType >> 1) & 15));
                    if (debug) log.debug("place order seq={} t={} orderId={} symbol={} uid={} price={} reserveBidPrice={} size={} userCookie={} {}/{} actionAndType={}", lastSeq, timestampNs, orderId, symbol, uid, price, reservedBidPrice, size, userCookie, orderAction, orderType, actionAndType);
                    ...
                    if (debug) log.debug("binary data seq={} t:{} {}", lastSeq, timestampNs, String.format("f=%d word0=%X word1=%X word2=%X word3=%X word4=%X", lastFlag, word0, word1, word2, word3, word4));
