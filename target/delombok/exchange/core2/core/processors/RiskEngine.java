...
public final class RiskEngine implements WriteBytesMarshallable {
    ...
    public static final ISerializationProcessor.SerializedModuleType MODULE_RE = ISerializationProcessor.SerializedModuleType.RISK_ENGINE;
    // state
    ...
    public RiskEngine(final int shardId, final long numShards, final ISerializationProcessor serializationProcessor, final SharedPool sharedPool, final ExchangeConfiguration exchangeConfiguration) {
        ...
        if (ISerializationProcessor.canLoadFromSnapshot(serializationProcessor, initStateCfg, shardId, MODULE_RE)) {
            ...
            final State state = serializationProcessor.loadData(initStateCfg.getSnapshotId(), MODULE_RE, shardId, bytesIn -> {
                ...
                final BinaryCommandsProcessor binaryCommandsProcessor = new BinaryCommandsProcessor(this::handleBinaryMessage, this::handleReportQuery, sharedPool, exchangeConfiguration.getReportsQueriesCfg(), bytesIn, shardId);
                ...
                return new State(symbolSpecificationProvider, userProfileService, binaryCommandsProcessor, lastPriceCache, fees, adjustments, suspends);
            });
            ...
            this.binaryCommandsProcessor = new BinaryCommandsProcessor(this::handleBinaryMessage, this::handleReportQuery, sharedPool, exchangeConfiguration.getReportsQueriesCfg(), shardId);
    ...
    public boolean preProcessCommand(final long seq, final OrderCommand cmd) {
        ...
        case BALANCE_ADJUSTMENT: 
            if (uidForThisHandler(cmd.uid)) {
                cmd.resultCode = adjustBalance(cmd.uid, cmd.symbol, cmd.price, cmd.orderId, BalanceAdjustmentType.of(cmd.orderType.getCode()));
            }
            return false;
        case SUSPEND_USER: 
            if (uidForThisHandler(cmd.uid)) {
        ...
        case PERSIST_STATE_RISK: 
            final boolean isSuccess = serializationProcessor.storeData(cmd.orderId, seq, cmd.timestamp, MODULE_RE, shardId, this);
    ...
    private CommandResultCode adjustBalance(long uid, int currency, long amountDiff, long fundingTransactionId, BalanceAdjustmentType adjustmentType) {
        ...
        if (res == CommandResultCode.SUCCESS) {
            ...
            case ADJUSTMENT: 
                // adjust total adjustments amount
                adjustments.addToValue(currency, -amountDiff);
                break;
            case SUSPEND: 
                // adjust total suspends amount
        ...
        if (message instanceof BatchAddSymbolsCommand) {
            ...
            ((BatchAddAccountsCommand) message).getUsers().forEachKeyValue((uid, accounts) -> {
                if (userProfileService.addEmptyUserProfile(uid)) {
                    accounts.forEachKeyValue((cur, bal) -> adjustBalance(uid, cur, bal, 1000000000 + cur, BalanceAdjustmentType.ADJUSTMENT));
                } else {
    ...
    private CommandResultCode placeOrder(final OrderCommand cmd, final UserProfile userProfile, final CoreSymbolSpecification spec) {
    ...
    private CommandResultCode placeExchangeOrder(final OrderCommand cmd, final UserProfile userProfile, final CoreSymbolSpecification spec) {
        ...
        if (cfgMarginTradingEnabled) {
            for (final SymbolPositionRecord position : userProfile.positions) {
                if (position.currency == currency) {
                    ...
                    freeFuturesMargin += (position.estimateProfit(spec2, lastPriceCache.get(recSymbol)) - position.calculateRequiredMarginForFutures(spec2));
                }
            }
        }
        ...
        if (logDebug) {
            log.debug("R1 uid={} : orderHoldAmount={} vs serProfile.accounts.get({})={} + freeFuturesMargin={}", userProfile.uid, orderHoldAmount, currency, userProfile.accounts.get(currency), freeFuturesMargin);
        }
...
//            log.warn("orderAmount={} > userProfile.accounts.get({})={}", orderAmount, currency, userProfile.accounts.get(currency));
     ...
     */
    private boolean canPlaceMarginOrder(final OrderCommand cmd, final UserProfile userProfile, final CoreSymbolSpecification spec, final SymbolPositionRecord position) {
...
//                newRequiredMarginForSymbol, position.currency, userProfile.accounts.get(position.currency), freeMargin);
    ...
    private void handleMatcherEventMargin(final MatcherTradeEvent ev, final CoreSymbolSpecification spec, final OrderAction takerAction, final UserProfile takerUp, final SymbolPositionRecord takerSpr) {
        if (takerUp != null) {
    ...
    private void handleMatcherRejectReduceEventExchange(final OrderCommand cmd, final MatcherTradeEvent ev, final CoreSymbolSpecification spec, final boolean takerSell, final UserProfile taker) {
    ...
    private void handleMatcherEventsExchangeSell(MatcherTradeEvent ev, final CoreSymbolSpecification spec, final UserProfile taker) {
    ...
    private void handleMatcherEventsExchangeBuy(MatcherTradeEvent ev, final CoreSymbolSpecification spec, final UserProfile taker, final OrderCommand cmd) {
    ...
    private static class State {
        ...
        @java.lang.SuppressWarnings("all")
        public State(final SymbolSpecificationProvider symbolSpecificationProvider, final UserProfileService userProfileService, final BinaryCommandsProcessor binaryCommandsProcessor, final IntObjectHashMap<LastPriceCacheRecord> lastPriceCache, final IntLongHashMap fees, final IntLongHashMap adjustments, final IntLongHashMap suspends) {
