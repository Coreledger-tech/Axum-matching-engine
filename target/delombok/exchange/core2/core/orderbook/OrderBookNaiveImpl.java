...
public final class OrderBookNaiveImpl implements IOrderBook {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(OrderBookNaiveImpl.class);
    ...
    public OrderBookNaiveImpl(final CoreSymbolSpecification symbolSpec, final ObjectsPool pool, final OrderBookEventsHelper eventsHelper, final LoggingConfiguration loggingCfg) {
    ...
    private void newOrderPlaceGtc(final OrderCommand cmd) {
        ...
        final Order orderRecord = new Order(newOrderId, price, size, filledSize, cmd.reserveBidPrice, action, cmd.uid, cmd.timestamp);
    ...
    private void newOrderMatchFokBudget(final OrderCommand cmd) {
        final long size = cmd.size;
        final SortedMap<Long, OrdersBucketNaive> subtreeForMatching = cmd.action == OrderAction.ASK ? bidBuckets : askBuckets;
     ...
     */
    private long tryMatchInstantly(final IOrder activeOrder, final SortedMap<Long, OrdersBucketNaive> matchingBuckets, long filled, final OrderCommand triggerCmd) {
