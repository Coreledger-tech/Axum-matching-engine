...
public final class OrderBookEventsHelper {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(OrderBookEventsHelper.class);
    ...
    public MatcherTradeEvent sendTradeEvent(final IOrder matchingOrder, final boolean makerCompleted, final boolean takerCompleted, final long size, final long bidderHoldPrice) {
...
//        event.activeOrderSeq = cmd.seq;
    ...
    public MatcherTradeEvent createBinaryEventsChain(final long timestamp, final int section, final NativeBytes<Void> bytes) {
...
//            log.debug("BIN EVENT: {}", event);
    ...
    public static NavigableMap<Integer, Wire> deserializeEvents(final OrderCommand cmd) {
        ...
        sections.forEach((section, events) -> {
            final long[] dataArray = events.stream().flatMap(evt -> Stream.of(evt.matchedOrderId, evt.matchedOrderUid, evt.price, evt.size, evt.bidderHoldPrice)).mapToLong(s -> s).toArray();
...
//            log.debug("UPDATED HEAD size={}", eventsChainHead == null ? 0 : eventsChainHead.getChainSize());
    ...
    @java.lang.SuppressWarnings("all")
    public OrderBookEventsHelper(final Supplier<MatcherTradeEvent> eventChainsSupplier) {
