...
public final class AffinityThreadFactory implements ThreadFactory {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(AffinityThreadFactory.class);
...
//            return Executors.defaultThreadFactory().newThread(runnable);
    ...
    private void executePinned(@NotNull Runnable runnable) {
        try (AffinityLock lock = getAffinityLockSync()) {
            ...
            log.debug("{} will be running on thread={} pinned to cpu {}", runnable, Thread.currentThread().getName(), lock.cpuId());
            runnable.run();
        } finally {
    ...
    private synchronized AffinityLock getAffinityLockSync() {
        return threadAffinityMode == ThreadAffinityMode.THREAD_AFFINITY_ENABLE_PER_PHYSICAL_CORE ? AffinityLock.acquireCore() : AffinityLock.acquireLock();
    }


    public enum ThreadAffinityMode {
        THREAD_AFFINITY_ENABLE_PER_PHYSICAL_CORE, THREAD_AFFINITY_ENABLE_PER_LOGICAL_CORE, THREAD_AFFINITY_DISABLE;
    }
