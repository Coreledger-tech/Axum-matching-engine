...
//        log.debug("COMPRESS originalSize={}", originalSize);
        ...
        return toLongsArray(byteBufferCompressed.array(), byteBufferCompressed.arrayOffset(), compressedBytesLen, padding);
    }
...
//        log.debug("long dataArray.len={} longsTransfered={}", dataArray.length, longsTransfered);
    ...
    public static <K, V> void marshallGenericMap(final Map<K, V> map, final BytesOut bytes, final BiConsumer<BytesOut, K> keyMarshaller, final BiConsumer<BytesOut, V> valMarshaller) {
        bytes.writeInt(map.size());
        map.forEach((k, v) -> {
    ...
    public static <K, V, M extends Map<K, V>> M readGenericMap(final BytesIn bytes, final Supplier<M> mapSupplier, final Function<BytesIn, K> keyCreator, final Function<BytesIn, V> valCreator) {
        int length = bytes.readInt();
        final M map = mapSupplier.get();
        for (int i = 0; i < length; i++) {
